...
setup
{
	...
	CREATE TABLE csn_ordinary_oldest_active_xid_state (
		label text PRIMARY KEY,
		pid int NOT NULL,
		fxid xid8
	);
	CREATE TABLE csn_ordinary_oldest_active_xid_data (id int PRIMARY KEY, val int);
	INSERT INTO csn_ordinary_oldest_active_xid_data VALUES (1, 0);
}
teardown
{
	DROP TABLE csn_ordinary_oldest_active_xid_data;
	DROP TABLE csn_ordinary_oldest_active_xid_state;
...
step reset
{
	TRUNCATE csn_ordinary_oldest_active_xid_state;
	UPDATE csn_ordinary_oldest_active_xid_data SET val = 0 WHERE id = 1;
}

session writer_before
step wb_seed
{
	INSERT INTO csn_ordinary_oldest_active_xid_state(label, pid, fxid)
...
step wb_prepare
{
	...
	UPDATE csn_ordinary_oldest_active_xid_data SET val = 1 WHERE id = 1;
}
...
step wa_prepare
{
	...
	SELECT pg_current_xact_id() IS NOT NULL AS writer_fxid_assigned;
	SELECT pg_advisory_lock_shared(1, pg_current_xact_id()::text::int4);
	UPDATE csn_ordinary_oldest_active_xid_data SET val = 2 WHERE id = 1;
}
...
{
	UPDATE csn_ordinary_oldest_active_xid_state
	SET fxid = (
...
step o_before_visible
{
	SELECT injection_points_oldest_active_xid(false, false) =
		(SELECT fxid
		 FROM csn_ordinary_oldest_active_xid_state
		 WHERE label = 'before') AS before_seen_by_oldest_active_reader;
}
