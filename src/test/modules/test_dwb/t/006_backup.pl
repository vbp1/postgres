...
# back on (the ring only repairs local torn writes; a backup copied mid-write
# can hold a torn page that just WAL replay with FPIs must repair).  The
# backup itself excludes pg_dwb/ contents, tolerates pg_dwb being a symlink,
# and a cluster restored from it cold-starts a fresh ring.
...
# would go; under double_writes none may appear.
$node->safe_psql('postgres', 'CHECKPOINT');
my $lsn0 = $node->safe_psql('postgres', 'SELECT pg_current_wal_insert_lsn()');
$node->safe_psql('postgres', "UPDATE dwb_fpi SET pad = repeat('a', 64) WHERE id = 1");
my $lsn1 = $node->safe_psql('postgres', 'SELECT pg_current_wal_insert_lsn()');
...
# pg_backup_start checkpoints and raises runningBackups; the next touch of
# the same page must now carry an FPI (doPageWrites =
...
my $lsn2 = $node->safe_psql('postgres', 'SELECT pg_current_wal_insert_lsn()');
$node->safe_psql('postgres', "UPDATE dwb_fpi SET pad = repeat('b', 64) WHERE id = 2");
my $lsn3 = $node->safe_psql('postgres', 'SELECT pg_current_wal_insert_lsn()');
...
like($waldump, qr/\bFPW\b/, 'an active backup forces full-page images');
